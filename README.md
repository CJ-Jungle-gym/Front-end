...
* **기술 스택** : React, Vite, TailwindCSS, Axios, React-Router, AWS Cognito
* **CI/CD 파이프라인** : GitHub Actions + Jenkins + ArgoCD + AWS ECR
* **배포 환경** :
* 운영계: **AWS EKS**
* 이벤트계: **AWS ECS**
* **특징** :
...
│   ├── api/                # API 요청 관리 (Axios)
│   │   ├── apiCenter.js    # 운영계 API (EKS)
│   │   ├── event.js        # 이벤트계 API (ECS)
│   ├── routes/Router.jsx   # React-Router 설정
│   ├── styles/             # 스타일 파일 (Tailwind + CSS)
│   └── main.jsx            # 앱 진입점
│── .env                    # 환경 변수 파일
│── package.json            # 패키지 정보
│── vite.config.js          # Vite 설정 파일
│── Jenkinsfile             # CI/CD Jenkins 스크립트
